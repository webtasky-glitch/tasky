...
export const ProjectsView: React.FC = () => {
  ...
  const handleJoinProject = async (e: React.FormEvent) => {
    ...
    if (!joinCodeInput.trim()) {
      setJoinStatus({ type: 'error', message: isEl ? 'Παρακαλούμε εισάγετε κωδικό συμμετοχής έργου.' : 'Please enter a project join code.' });
      return;
    }
  ...
  const handleAddProjectTask = (e: React.FormEvent) => {
    ...
    addTask({
      ...
      description: isEl ? `Εργασία για το έργο ${activeProject.name}` : `Task for project ${activeProject.name}`,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-neutral-200/20 dark:border-white/5 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-white">
                {isEl ? 'Χώρος Έργων' : 'Projects Workspace'}
              </h1>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                {isEl ? 'Δημιουργήστε έργα, παράγετε κωδικούς συμμετοχής και συνεργαστείτε απρόσκοπτα.' : 'Create custom projects, generate join codes, and collaborate seamlessly.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {isEl ? 'Σύνδεση με Κωδικό' : 'Join with Code'}
          </button>
          ...
          >
            ...
            {isEl ? 'Δημιουργία Έργου' : 'Create Project'}
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-4 overflow-hidden border-r border-neutral-200/20 dark:border-white/5 pr-0 lg:pr-6">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-neutral-400 dark:text-neutral-500">
              {isEl ? 'Τα Έργα μου' : 'My Projects'} ({userProjects.length})
            </span>
            <div className="relative">
              <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-neutral-400" />
              <input
                type="text"
                placeholder={isEl ? 'Φίλτρο...' : 'Filter...'}
                value={searchQuery}
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            {filteredProjects.length === 0 ? (
              <div className="text-center py-12 px-4 bg-white/20 dark:bg-neutral-800/20 rounded-2xl border border-dashed border-neutral-300 dark:border-white/10">
                ...
                <p className="text-xs font-medium text-neutral-600 dark:text-neutral-400">{isEl ? 'Δεν βρέθηκαν έργα' : 'No projects found'}</p>
                <p className="text-[11px] text-neutral-400 dark:text-neutral-500 mt-1">
                  {isEl ? 'Δημιουργήστε νέο έργο ή συνδεθείτε χρησιμοποιώντας έναν κωδικό συμμετοχής.' : "Create a new project or join one using a friend's join code."}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between mt-3 pt-2 border-t border-neutral-200/20 dark:border-white/5 text-[11px] text-neutral-500 dark:text-neutral-400">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          title={isEl ? 'Αντιγραφή κωδικού' : 'Copy join code'}
                        >
                          {copiedCode === p.code ? (
        ...
        <div className="lg:col-span-8 flex flex-col overflow-y-auto space-y-6">
          {activeProject ? (
                  ...
                  <div className="p-3 bg-white/80 dark:bg-neutral-900/80 rounded-2xl border border-indigo-500/30 flex items-center gap-3 shrink-0 shadow-md">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-400 block">
                        {isEl ? 'Κωδικός Συμμετοχής' : 'Project Join Code'}
                      </span>
                    ...
                    <button
                      ...
                      title={isEl ? 'Αντιγραφή κωδικού για κοινή χρήση' : 'Copy code to share with teammates'}
                    >
                      {copiedCode === activeProject.code ? (
                        <>
                          <Check className="w-4 h-4 text-emerald-500" />
                          <span className="text-emerald-600 dark:text-emerald-400">{isEl ? 'Αντιγράφηκε!' : 'Copied!'}</span>
                        </>
                      ) : (
                        <>
                          <Copy className="w-4 h-4" />
                          <span>{isEl ? 'Αντιγραφή' : 'Copy'}</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-4 border-t border-neutral-200/30 dark:border-white/5">
                  <div className="flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
                    ...
                    <span className="font-bold text-neutral-800 dark:text-neutral-200">
                      {activeProject.memberIds?.length || 1} {isEl ? 'Μέλη συνδέθηκαν' : 'Members Joined'}
                    </span>
                    <span className="text-neutral-300 dark:text-neutral-600">•</span>
                    <span>{isEl ? 'Δημιουργήθηκε από' : 'Created by'} {activeProject.ownerName || (isEl ? 'Ιδιοκτήτη' : 'Project Owner')}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {activeProject.ownerId === currentUserId ? (
                      <button
                        onClick={() => {
                          if (window.confirm(isEl ? `Είστε βέβαιοι ότι θέλετε να διαγράψετε το έργο "${activeProject.name}";` : `Are you sure you want to delete project "${activeProject.name}"?`)) {
                      ...
                      >
                        ...
                        {isEl ? 'Διαγραφή Έργου' : 'Delete Project'}
                      </button>
                    ) : (
                      <button
                        onClick={() => {
                          if (window.confirm(isEl ? `Είστε βέβαιοι ότι θέλετε να αποχωρήσετε από το έργο "${activeProject.name}";` : `Are you sure you want to leave project "${activeProject.name}"?`)) {
              ...
              <div className="bg-white/40 dark:bg-neutral-800/40 rounded-2xl p-4 border border-neutral-200/40 dark:border-white/5 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 flex items-center gap-2">
                    ...
                    {isEl ? `Προσθήκη Εργασίας στο ${activeProject.name}` : `Add Task to ${activeProject.name}`}
                  </h3>
                  <span className="text-[11px] text-indigo-600 dark:text-indigo-400 font-medium">
                    {isEl ? `Αναθέσιμο σε οποιοδήποτε μέλος (${activeProjectMembers.length})` : `Assignable to any member in this project (${activeProjectMembers.length})`}
                  </span>
                </div>
                ...
                <form onSubmit={handleAddProjectTask} className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                    <div className="sm:col-span-6">
                      <input
                        type="text"
                        placeholder={isEl ? 'Τίτλος εργασίας έργου...' : 'Project task title...'}
                        value={newTaskTitle}
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="flex-1 w-full">
                      <AssigneeMultiSelector
                        ...
                        placeholder={isEl ? 'Ανάθεση σε μέλος του έργου (ανεξαρτήτως πλάνου)...' : 'Assign any project member (cross-plan)...'}
                      />
                    </div>
                    <button
                      type="submit"
                    ...
                    >
                      <Plus className="w-4 h-4" />
                      {isEl ? 'Προσθήκη Εργασίας' : 'Add Project Task'}
                    </button>
                  </div>
                </form>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 flex items-center gap-2">
                    ...
                    {isEl ? 'Εργασίες Έργου' : 'Project Tasks'} ({projectTasks.length})
                  </h3>
                </div>
                ...
                {projectTasks.length === 0 ? (
                  <div className="p-8 text-center bg-white/20 dark:bg-neutral-800/20 rounded-2xl border border-dashed border-neutral-300 dark:border-white/10">
                    ...
                    <p className="text-xs font-semibold text-neutral-600 dark:text-neutral-300">{isEl ? 'Δεν υπάρχουν εργασίες σε αυτό το έργο ακόμα' : 'No tasks in this project yet'}</p>
                    <p className="text-[11px] text-neutral-400 mt-1">{isEl ? 'Χρησιμοποιήστε τη φόρμα παραπάνω για να αναθέσετε εργασίες.' : 'Use the input form above to assign tasks to this project workspace.'}</p>
                  </div>
                ) : (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-20 px-4 text-center">
              ...
              <h2 className="text-lg font-bold text-neutral-800 dark:text-white">{isEl ? 'Επιλέξτε ή Δημιουργήστε Έργο' : 'Select or Create a Project'}</h2>
              <p className="text-xs text-neutral-500 max-w-sm mt-1 mb-6">
                {isEl ? 'Συνδεθείτε σε ένα υπάρχον έργο με κωδικό συμμετοχής ή δημιουργήστε έναν νέο χώρο έργου για την ομάδα σας.' : 'Join an existing project with a join code or create a new project workspace to share with your team.'}
              </p>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setIsJoinModalOpen(true)}
                ...
                >
                  {isEl ? 'Σύνδεση με Κωδικό' : 'Join Code'}
                </button>
                <button
                  onClick={() => setIsCreateModalOpen(true)}
            ...
            >
              <div className="flex items-center justify-between pb-3 border-b border-neutral-200 dark:border-white/5">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-bold text-base text-neutral-900 dark:text-white">{isEl ? 'Σύνδεση σε Έργο' : 'Join a Project'}</h3>
                </div>
                <button 
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                {isEl ? 'Εισαγάγετε τον 6-ψήφιο κωδικό συμμετοχής έργου που σας δόθηκε.' : 'Enter the 6-character project join code provided by the project creator.'}
              </p>
              ...
              <form onSubmit={handleJoinProject} className="space-y-4">
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-neutral-400 block mb-1">
                    {isEl ? 'Κωδικός Έργου' : 'Project Code'}
                  </label>
                  <input
                    type="text"
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  >
                    {isEl ? 'Σύνδεση σε Έργο' : 'Join Project'}
                  </button>
                </div>
              </form>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between pb-3 border-b border-neutral-200 dark:border-white/5">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-bold text-base text-neutral-900 dark:text-white">{isEl ? 'Δημιουργία Νέου Έργου' : 'Create New Project'}</h3>
                </div>
                <button 
              ...
              <form onSubmit={handleCreateProject} className="space-y-4">
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-neutral-400 block mb-1">
                    {isEl ? 'Όνομα Έργου' : 'Project Name'}
                  </label>
                  <input
                    type="text"
                    placeholder={isEl ? 'π.χ. Επανασχεδιασμός Ιστοσελίδας 2026' : 'e.g. Website Redesign 2026'}
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-neutral-400 block mb-1">
                    {isEl ? 'Περιγραφή (Προαιρετικό)' : 'Description (Optional)'}
                  </label>
                  <textarea
                    placeholder={isEl ? 'Σύντομος στόχος ή περιγραφή...' : 'Brief objective or description...'}
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-neutral-400 block mb-1">
                    {isEl ? 'Χρώμα Θέματος' : 'Theme Color'}
                  </label>
                  <div className="flex items-center gap-3">
                ...
                <div className="p-3 bg-indigo-500/10 rounded-2xl border border-indigo-500/20 text-xs text-indigo-600 dark:text-indigo-300">
                  <span className="font-bold">{isEl ? 'Αυτόματος Κωδικός Συμμετοχής:' : 'Automatic Join Code:'}</span> {isEl ? 'Ένας μοναδικός κωδικός 6 χαρακτήρων θα δημιουργηθεί αυτόματα ώστε να μπορούν άλλοι να συνδεθούν άμεσα!' : 'A unique 6-character code will be generated upon creation so others can join this project instantly!'}
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-2xl text-xs font-bold transition-all shadow-md cursor-pointer"
                  >
                    {isEl ? 'Δημιουργία & Παραγωγή Κωδικού' : 'Create & Generate Code'}
                  </button>
                </div>
              </form>
            </motion.div>
