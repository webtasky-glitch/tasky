...
export const HabitsView: React.FC = () => {
  ...
  const getDayLetter = (date: Date) => {
    ...
    const daysEl = ['Κ', 'Δ', 'Τ', 'Τ', 'Π', 'Π', 'Σ'];
    const days = isEl ? daysEl : daysEn;
    return days[date.getDay()];
  };

  const getDayFormatted = (date: Date) => {
    return date.toLocaleDateString(isEl ? 'el-GR' : 'en-US', { month: 'short', day: 'numeric' });
  };
    ...
    <div className="flex-1 glass-panel rounded-[32px] p-6 sm:p-8 space-y-6 overflow-y-auto shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-sans font-bold text-neutral-900 dark:text-white tracking-tight">
            {isEl ? 'Στόχοι & Παρακολούθηση Συνηθειών' : 'Goals & Habit Tracker'}
          </h1>
          <p className="text-sm text-neutral-500">
            {isEl ? 'Χτίστε σταθερές ρουτίνες, παρακολουθήστε σερί και καθιερώστε υγιείς ρυθμούς εργασίας.' : 'Build consistent routines, monitor streaks, and establish healthy work-life patterns.'}
          </p>
        </div>
        ...
        >
          ...
          {isEl ? 'Δημιουργία Νέου Στόχου' : 'Create New Goal'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 p-4 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-widest font-mono">{isEl ? 'Ενεργοί Στόχοι' : 'Active Goals'}</p>
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 p-4 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-widest font-mono">{isEl ? 'Καλύτερο Σερί' : 'Best Streak'}</p>
            <p className="text-lg font-bold text-neutral-800 dark:text-neutral-100">{maxStreak} {isEl ? 'Ημέρες' : 'Days'}</p>
          </div>
        </div>
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 p-4 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-widest font-mono">{isEl ? 'Ολοκληρώθηκαν Σήμερα' : 'Completed Today'}</p>
            <p className="text-lg font-bold text-neutral-800 dark:text-neutral-100">
              {completedTodayCount} {isEl ? 'από' : 'of'} {activeHabitsCount}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-neutral-800 dark:text-white text-sm">{isEl ? 'Δημιουργία Νέου Στόχου Ρουτίνας' : 'Create New Routine Goal'}</h3>
              ...
              <form onSubmit={handleCreateHabit} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-neutral-600 dark:text-neutral-400">{isEl ? 'Όνομα Στόχου / Συνήθειας' : 'Goal / Habit Name'}</label>
                  <input 
                    type="text" 
                    required 
                    placeholder={isEl ? 'π.χ. Προγραμματισμός για 30 λεπτά' : 'e.g. Code for 30 minutes'}
                    value={habitName}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-neutral-600 dark:text-neutral-400">{isEl ? 'Συχνότητα' : 'Frequency'}</label>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      {isEl ? 'Καθημερινή Ρουτίνα' : 'Daily Routine'}
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      {isEl ? 'Εβδομαδιαίος Έλεγχος' : 'Weekly Checkin'}
                    </button>
                  </div>
                </div>
                ...
                >
                  {isEl ? 'Έναρξη Παρακολούθησης' : 'Start Tracking Goal'}
                </button>
              </form>
            </motion.div>
          </div>
      ...
      <div className="bg-white/20 dark:bg-white/5 border border-white/30 dark:border-white/10 rounded-2xl p-5 shadow-sm space-y-4 backdrop-blur-md">
        <h2 className="text-sm font-semibold text-neutral-800 dark:text-neutral-100 flex items-center gap-1.5 pb-3 border-b border-neutral-200/20 dark:border-white/5">
          ...
          {isEl ? 'Εβδομαδιαίος Πίνακας Προόδου Ρουτίνας' : 'Weekly Routine Progress Grid'}
        </h2>
        ...
        {habits.length > 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-400 font-mono font-medium">
                      {habit.frequency === 'Daily' ? (isEl ? 'Καθημερινά' : 'Daily') : (isEl ? 'Εβδομαδιαία' : 'Weekly')}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end overflow-x-auto pb-1 md:pb-0">
                    ...
                    {last7Days.map((day, dIdx) => {
                      ...
                      return (
                        <button
                          ...
                          title={`${getDayFormatted(day)}: ${isCompleted ? (isEl ? 'Ολοκληρώθηκε' : 'Completed') : (isEl ? 'Εκκρεμεί' : 'Pending')}`}
                  ...
                  {confirmDeleteId === habit.id ? (
                    <div className="flex items-center gap-1.5 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 px-2 py-1 rounded-xl shrink-0">
                      <span className="text-[10px] font-medium text-rose-700 dark:text-rose-300">{isEl ? 'Διαγραφή;' : 'Delete?'}</span>
                      <button 
                        onClick={() => {
                    ...
                    <button
                      ...
                      title={isEl ? 'Διαγραφή στόχου' : 'Delete goal'}
                    >
          ...
          <div className="text-center py-12 border border-dashed border-white/30 dark:border-white/10 rounded-2xl bg-white/5 dark:bg-white/2">
            <p className="text-sm text-neutral-500 italic">{isEl ? 'Δεν υπάρχουν ακόμη στόχοι ρουτίνας. Δημιουργήστε έναν στόχο για να αποκτήσετε θετική ορμή!' : 'No habit goals started yet. Create a goal to build positive momentum!'}</p>
            <button 
              onClick={() => setShowAddForm(true)}
              className="mt-3 text-xs text-indigo-600 dark:text-indigo-400 font-bold hover:underline cursor-pointer"
            >
              {isEl ? 'Ξεκινήστε την Πρώτη σας Συνήθεια' : 'Start Your First Habit'}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="bg-white/10 dark:bg-black/20 border border-white/20 dark:border-white/5 p-4 rounded-xl text-xs text-neutral-500 leading-relaxed font-sans space-y-1.5">
        <h4 className="font-semibold text-neutral-700 dark:text-neutral-300">{isEl ? 'Πώς καταγράφονται τα σερί:' : 'How streaks are tracked:'}</h4>
        <p>{isEl ? 'Ένα σερί ενημερώνεται κάθε φορά που ολοκληρώνετε έναν στόχο σε συνεχόμενες ημέρες. Η ενεργοποίηση ή απενεργοποίηση ημερομηνιών επανυπολογίζει αυτόματα τα σερί. Η παράλειψη μιας ημέρας μηδενίζει τον μετρητή σερί, τονίζοντας την καθημερινή εστίαση και τη συνέπεια.' : 'A streak is updated whenever you complete a goal on consecutive days. Toggling dates on or off will recalculate streaks automatically. Missing a day resets the streak counter to zero, emphasizing daily focus and long-term consistency.'}</p>
      </div>
    </div>
  );
};
