...
export const TimelineScheduleView: React.FC = () => {
  ...
  // Filter to show ONLY tasks assigned to the current user (defaults to true)
  const [onlyAssignedToMe, setOnlyAssignedToMe] = useState<boolean>(() => {
    const saved = localStorage.getItem('tasky_timeline_only_me');
    return saved !== null ? saved === 'true' : true;
  });

  const handleToggleOnlyAssignedToMe = () => {
    setOnlyAssignedToMe(prev => {
      const next = !prev;
      localStorage.setItem('tasky_timeline_only_me', String(next));
      return next;
    });
  };
  ...
  // Helper to get organization for task - strictly checks only the plan it explicitly belongs to
  ...
  const isTaskAssignedToMe = (t: Task): boolean => {
    ...
    // 1. Check multiple assignees array
    if (t.assignedToIds && Array.isArray(t.assignedToIds) && t.assignedToIds.length > 0) {
      ...
      const matchesMember = t.assignedToIds.some(id => {
    ...
    // 2. Check primary / legacy assignee
    if (t.assignedTo) {
    ...
    // 3. Fallback: if task has no assignee at all, check if created by user
    if (!t.assignedTo && (!t.assignedToIds || t.assignedToIds.length === 0)) {
      if (myId && t.createdBy === myId) return true;
      if (myUid && (t as any).userId === myUid) return true;
    }

    return false;
  };

  // Filter tasks by assigned-to-me and selected plan
  const visibleTasks = useMemo(() => {
    return tasks.filter((t: Task) => {
      // Filter out tasks not assigned to me when onlyAssignedToMe is active
      if (onlyAssignedToMe && !isTaskAssignedToMe(t)) {
        return false;
      }
      if (selectedPlanId === 'all') return true;
      if (selectedPlanId === 'personal') {
  ...
  }, [tasks, selectedPlanId, onlyAssignedToMe, userOrganizations, teamMembers, currentUserProfile, user]);

  // Calculate task counts per plan option (respecting onlyAssignedToMe filter)
  const planTaskCounts = useMemo(() => {
    const baseTasks = tasks.filter((t: Task) => !onlyAssignedToMe || isTaskAssignedToMe(t));
    const counts: Record<string, number> = {
      all: baseTasks.length,
      personal: 0
    };
    userOrganizations.forEach((org: Organization) => {
      counts[org.id] = 0;
    });

    baseTasks.forEach((t: Task) => {
      const org = getTaskOrg(t);
      if (org && counts[org.id] !== undefined) {
        counts[org.id]++;
      } else {
    ...
    return counts;
  }, [tasks, onlyAssignedToMe, userOrganizations, teamMembers, currentUserProfile, user]);
  ...
  const openSlotScheduler = (dateStr: string, hourInt: number) => {
    ...
    setNewTaskPlanId(selectedPlanId !== 'all' ? selectedPlanId : (userOrganizations[0]?.id || ''));
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-200/40 dark:border-white/10 pb-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {/* Only Assigned to Me Toggle Button */}
          <button
            type="button"
            onClick={handleToggleOnlyAssignedToMe}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer border ${
              onlyAssignedToMe
                ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs'
                : 'bg-white/70 dark:bg-white/5 text-neutral-700 dark:text-neutral-300 border-neutral-200/60 dark:border-white/10 hover:bg-neutral-100 dark:hover:bg-white/10'
            }`}
            title={onlyAssignedToMe ? (isEl ? 'Εμφάνιση μόνο των δικών μου εργασιών' : 'Showing only tasks assigned to you') : (isEl ? 'Εμφάνιση όλων των εργασιών' : 'Showing all tasks')}
          >
            <UserCheck className="w-3.5 h-3.5" />
            <span>{onlyAssignedToMe ? (isEl ? 'Μόνο ανατεθειμένες σε μένα' : 'Assigned to Me') : (isEl ? 'Όλες οι εργασίες' : 'All Tasks')}</span>
          </button>
          ...
          <div className="flex items-center gap-1.5 bg-white/70 dark:bg-white/5 border border-neutral-200/60 dark:border-white/10 px-3 py-1.5 rounded-xl text-xs">
            ...
            >
              <option value="all">{t('timeline.allPlans')} ({planTaskCounts.all ?? 0})</option>
              {organizations.map((org: Organization) => (
                <option key={org.id} value={org.id}>{org.name} ({planTaskCounts[org.id] || 0})</option>
              ))}
              {(planTaskCounts.personal || 0) > 0 && (
                <option value="personal">{isEl ? 'Προσωπικές (Χωρίς Πλάνο)' : 'Personal (No Plan)'} ({planTaskCounts.personal})</option>
              )}
            </select>
          </div>
      ...
      {/* DEDICATED PLAN SELECTOR SECTION: All Tasks vs Every Individual Plan */}
      <div className="bg-white/50 dark:bg-white/5 border border-neutral-200/70 dark:border-white/10 rounded-2xl p-3 sm:p-3.5 space-y-2.5 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs sm:text-sm font-bold text-neutral-800 dark:text-white">
                  {isEl ? 'Επιλογή Πλάνου στο Χρονοδιάγραμμα:' : 'Timeline Plan Filter:'}
                </span>
                <span className="text-[11px] text-neutral-500 dark:text-neutral-400">
                  {isEl ? 'Όλες οι εργασίες ή ανά μεμονωμένο πλάνο' : 'View all tasks or every individual plan'}
                </span>
              </div>
            </div>
          </div>
          ...
          {selectedPlanId !== 'all' && (
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 rounded-lg transition-colors self-start sm:self-auto cursor-pointer"
              title={isEl ? 'Εμφάνιση όλων των πλάνων' : 'Show all plans'}
            >
              <X className="w-3.5 h-3.5" />
              <span>{isEl ? 'Επαναφορά σε Όλες' : 'Reset to All Tasks'}</span>
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer border ${
              selectedPlanId === 'all'
                ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white border-indigo-600 shadow-sm shadow-indigo-500/20 ring-2 ring-indigo-400/40'
                : 'bg-white/80 dark:bg-white/5 text-neutral-700 dark:text-neutral-300 border-neutral-200/70 dark:border-white/10 hover:bg-neutral-100 dark:hover:bg-white/10'
            }`}
          >
            <Layers className="w-3.5 h-3.5" />
            <span>{isEl ? 'Όλες οι Εργασίες' : 'All Tasks (All Plans)'}</span>
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${
              selectedPlanId === 'all'
                ? 'bg-white/20 text-white'
                : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300'
            }`}>
              {planTaskCounts.all ?? 0}
            </span>
          </button>

          {/* 2. Every Individual Plan */}
          {userOrganizations.map((org: Organization) => {
            const isSelected = selectedPlanId === org.id;
            const count = planTaskCounts[org.id] || 0;
            const themeColor = org.themeColor || '#6366f1';

            return (
              <button
                key={org.id}
                type="button"
                onClick={() => setSelectedPlanId(org.id)}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer border ${
                  isSelected
                    ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm shadow-indigo-500/20 ring-2 ring-indigo-400/40'
                    : 'bg-white/80 dark:bg-white/5 text-neutral-700 dark:text-neutral-300 border-neutral-200/70 dark:border-white/10 hover:bg-neutral-100 dark:hover:bg-white/10'
                }`}
                title={`${org.name} (${count} ${isEl ? 'εργασίες' : 'tasks'})`}
              >
                <span
          ...
          {/* 3. Personal Tasks (No Plan) */}
          {((planTaskCounts.personal || 0) > 0 || userOrganizations.length === 0) && (
            <button
              type="button"
              onClick={() => setSelectedPlanId('personal')}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer border ${
            ...
            >
              ...
              <span>{isEl ? 'Προσωπικές (Χωρίς Πλάνο)' : 'Personal (No Plan)'}</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${
                selectedPlanId === 'personal'
                  ? 'bg-white/20 text-white'
                  : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300'
              }`}>
                {planTaskCounts.personal || 0}
              </span>
            </button>
          )}
        </div>

        {/* Informational banner when an individual plan is selected */}
        {selectedPlanId !== 'all' && (
          <div className="flex items-center justify-between text-xs py-1 px-2.5 bg-indigo-50/70 dark:bg-indigo-950/40 rounded-xl text-indigo-700 dark:text-indigo-300 border border-indigo-200/60 dark:border-indigo-800/40">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse" />
              <span>
                {selectedPlanId === 'personal' 
                  ? (isEl ? 'Προβολή μόνο προσωπικών εργασιών (εκτός πλάνων)' : 'Viewing only personal tasks outside of plans')
                  : (isEl 
                      ? `Προβολή αποκλειστικά για το πλάνο: ${organizations.find((o: Organization) => o.id === selectedPlanId)?.name || ''}`
                      : `Filtered to plan: ${organizations.find((o: Organization) => o.id === selectedPlanId)?.name || ''}`
                    )
                }
              </span>
            </div>
            <span className="font-mono text-[11px] font-bold">
              {visibleTasks.length} {isEl ? 'στο χρονοδιάγραμμα' : 'in schedule'}
            </span>
          </div>
        )}
      </div>
