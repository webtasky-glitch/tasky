...
export const TimelineScheduleView: React.FC = () => {
  ...
  const myOrgId = currentUserProfile?.orgId || (userOrganizations && userOrganizations.length > 0 ? userOrganizations[0]?.id : undefined);
  const myOrg = useMemo(() => {
    return organizations.find((o: Organization) => o.id === myOrgId);
  ...
  // Filter tasks to show ONLY the plan the user belongs to
  const visibleTasks = useMemo(() => {
    return tasks.filter((t: Task) => {
      ...
      if (myOrgId) {
        return org?.id === myOrgId;
      } else {
        // If they don't belong to any organization, show personal tasks (no org)
        return !org;
      }
    });
  }, [tasks, myOrgId, onlyAssignedToMe, teamMembers, currentUserProfile, user]);
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-200/40 dark:border-white/10 pb-6">
        <div>
          ...
          <div className="mt-2.5 inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/30">
            <Building2 className="w-3.5 h-3.5 text-indigo-500" />
            <span>
              {isEl ? 'Πλάνο στο οποίο ανήκετε:' : 'Active Plan (Your Organization):'} <span className="underline decoration-indigo-400/40">{myOrg ? myOrg.name : (isEl ? 'Προσωπικός Χώρος (Χωρίς Πλάνο)' : 'Personal Workspace (No Plan)')}</span>
            </span>
          </div>
        </div>
