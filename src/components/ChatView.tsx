...
export const ChatView: React.FC = () => {
        ...
        if (isAdmin) {
          ...
          if (selectedSupportUser) {
            ...
            const lastUserQuestion = userQuestions.length > 0 
              ...
              : (isEl ? 'Ερώτηση υποστήριξης χρήστη' : 'User support inquiry');
  ...
  const handleConfirmSaveAutoReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveSuccessToast(
        isEl 
          ? `Αποθηκεύτηκε η απάντηση στις Αυτόματες Απαντήσεις AI!`
          : `Saved "${pendingAutoReply.question.slice(0, 30)}..." to AI Auto-Replies!`
      );
  ...
  const getOrgName = (orgId?: string) => {
    if (!orgId) return isEl ? 'Προσωπικός Χώρος' : 'Personal Workspace';
    const o = organizations.find((org: Organization) => org.id === orgId);
    return o ? `${o.name} (${o.type})` : (isEl ? 'Ιδιωτικός Χώρος' : 'Private Workspace');
  };
  ...
  return (
    <div className="flex-1 glass-panel rounded-[32px] overflow-hidden flex h-full shadow-xl select-none relative">
      ...
      <div className={`
        fixed inset-y-0 left-0 z-40 w-72 sm:w-80 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl md:static md:w-64 lg:w-72 border-r border-neutral-200/20 dark:border-white/5 flex flex-col h-full shrink-0 transition-transform duration-300 ease-in-out md:translate-x-0
        ${isMobileChannelsOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full md:translate-x-0'}
      `}>
        <div className="p-4 sm:p-5 border-b border-neutral-200/20 dark:border-white/5 flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-neutral-800 dark:text-white flex items-center gap-2">
              <MessageSquare className="w-5 h-5 text-indigo-500" />
              {isEl ? 'Συνομιλία Χώρου Εργασίας' : 'Workspace Chat'}
            </h3>
            <p className="text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5">
              {isEl ? 'Μηνύματα σε πραγματικό χρόνο & AI υποστήριξη.' : 'Realtime messaging & AI auto-support.'}
            </p>
          </div>
          <button
            type="button"
            onClick={() => setIsMobileChannelsOpen(false)}
            className="md:hidden p-1.5 rounded-xl text-neutral-400 hover:text-neutral-700 dark:hover:text-white hover:bg-neutral-200/50 dark:hover:bg-white/10 cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div className="space-y-1">
            ...
            {myOrgId && (
              ...
              >
                ...
                <span className="truncate">#{myOrg ? myOrg.name : (isEl ? 'Συνομιλία Οργανισμού' : 'Company Chat')}</span>
              </button>
            )}
            ...
            {!isAdmin && (
              ...
              >
                ...
                <span>{isEl ? 'Τεχνική Υποστήριξη (AI + Admin)' : 'Tech Support (AI + Admin)'}</span>
              </button>
            )}
            ...
            {isAdmin && (
              ...
              >
                <div className="flex items-center gap-2.5 truncate">
                  ...
                  <span className="truncate font-bold">{isEl ? 'Αποθηκευμένες Απαντήσεις' : 'Saved Auto-Replies'}</span>
                </div>
          ...
          {isAdmin && (
            <div className="space-y-1.5 pt-2">
              <span className="text-[10px] font-bold text-rose-400 dark:text-rose-500 px-3 uppercase tracking-wider block mb-1 flex items-center gap-1">
                ...
                {isEl ? 'ΑΙΤΗΜΑΤΑ ΒΟΗΘΕΙΑΣ' : 'USER HELP TICKETS'}
              </span>
              {supportUsersList.length === 0 ? (
                <p className="text-[10px] text-neutral-400 italic px-3 pt-1">
                  {isEl ? 'Κανένα ενεργό αίτημα χρήστη.' : 'No active user tickets.'}
                </p>
              ) : (
        ...
        <div className="p-4 border-t border-neutral-200/20 dark:border-white/5 flex items-center gap-2.5 bg-black/5 dark:bg-black/15">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[9px] font-mono text-neutral-400 dark:text-neutral-500 truncate">
              {currentUserProfile?.role || (isEl ? 'Μέλος Χώρου' : 'Workspace User')}
            </p>
          </div>
        </div>
      </div>
      ...
      {activeChannel === 'saved-answers' ? (
        ...
        <div className="flex-1 flex flex-col h-full bg-white/5 dark:bg-white/1 overflow-hidden min-w-0">
          
          {/* Chat Room Header */}
          <div className="px-4 sm:px-6 py-3.5 sm:py-4.5 border-b border-neutral-200/20 dark:border-white/5 flex items-center justify-between bg-white/30 dark:bg-black/20 gap-2">
            <div className="flex items-center gap-2.5 min-w-0">
              {/* Mobile Channels Toggle Button */}
              <button
                type="button"
                onClick={() => setIsMobileChannelsOpen(true)}
                className="md:hidden p-2 rounded-xl bg-white/60 dark:bg-neutral-800/60 border border-neutral-200/50 dark:border-white/10 text-neutral-700 dark:text-neutral-200 shrink-0 cursor-pointer shadow-sm"
                title={isEl ? 'Προβολή καναλιών και αιτημάτων' : 'View channels and tickets'}
              >
                <Users className="w-4 h-4" />
              </button>

              <div className="min-w-0">
                <h4 className="text-xs sm:text-sm font-bold text-neutral-800 dark:text-white flex items-center gap-1.5 flex-wrap">
                  {activeChannel === 'company' ? (
                    <>
                    ...
                    <>
                      ...
                      <span className="truncate">
                        {isAdmin 
                          ? `${isEl ? 'Υποστήριξη:' : 'Tech Support:'} ${teamMembers.find((t: any) => t.id === selectedSupportUser)?.name || (isEl ? 'Επιλογή Αιτήματος' : 'Select Ticket')}`
                          : (isEl ? 'Τεχνική Υποστήριξη (Βοήθεια)' : 'Technical Support (Help Desk)')}
                      </span>
                      <span className="bg-indigo-500/10 text-indigo-500 text-[9px] px-2 py-0.5 rounded-full flex items-center gap-1 font-mono font-bold shrink-0">
                        <Sparkles className="w-3 h-3 text-indigo-400" />
                        {isEl ? 'Αυτόματη Απάντηση AI' : 'AI Auto-Responder'} ({aiSupportQA?.length || 0})
                      </span>
                    </>
                  )}
                </h4>
                <p className="text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5 truncate hidden sm:block">
                  {activeChannel === 'company'
                    ? (isEl ? 'Καλώς ήρθατε στο κανάλι του οργανισμού σας. Μόνο μέλη του οργανισμού μπορούν να διαβάσουν και να γράψουν εδώ.' : `Welcome to your official company channel. Only members in your organization can read and write here.`)
                    : isAdmin
                    ? (isEl ? 'Παροχή άμεσης τεχνικής βοήθειας. Κάθε απάντηση που στέλνετε μπορεί να αποθηκευτεί ως αυτόματη απάντηση.' : 'Providing responsive live technical assistance. Every answer you send can be saved into the auto-reply engine.')
                    : (isEl ? 'Απευθείας γραμμή τεχνικής υποστήριξης. Άμεσες απαντήσεις με τη βοήθεια του Tasky AI.' : 'Direct encrypted line to administrative tech support. Instant answers powered by Tasky AI.')}
                </p>
              </div>
            </div>
            ...
            {isAdmin && (
              <button
                ...
                className="px-2.5 sm:px-3 py-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-300 border border-indigo-200/40 dark:border-indigo-800/40 rounded-xl text-[11px] sm:text-xs font-semibold flex items-center gap-1.5 hover:bg-indigo-100 transition-all cursor-pointer shrink-0"
              >
                <Bot className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">{isEl ? 'Βιβλιοθήκη Απαντήσεων' : 'Auto-Reply Library'}</span>
                <span className="sm:hidden">{isEl ? 'Βιβλιοθήκη' : 'Library'}</span>
              </button>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {activeChannel === 'support' && isAdmin && !selectedSupportUser ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-neutral-400 dark:text-neutral-500 space-y-3">
                ...
                <p className="text-xs font-bold font-sans">{isEl ? 'Δεν έχει επιλεγεί αίτημα υποστήριξης' : 'No Support Ticket Selected'}</p>
                <p className="text-[10px] max-w-xs">{isEl ? 'Επιλέξτε ένα αίτημα χρήστη από τον πίνακα υποστήριξης στην πλαϊνή μπάρα για να συνομιλήσετε ή να δημιουργήσετε αυτοματοποιημένες απαντήσεις.' : 'Select a user\'s help ticket from the support panel in the sidebar to review conversations, reply, and build automated replies.'}</p>
                <button
                  onClick={() => setActiveChannel('saved-answers')}
                ...
                >
                  ...
                  {isEl ? 'Διαχείριση Αυτόματων Απαντήσεων' : 'Manage Auto-Reply Answers'}
                </button>
              </div>
            ) : (
              <>
                          ...
                          <span className="font-bold text-neutral-700 dark:text-neutral-300 flex items-center gap-1">
                            ...
                            {isAI && (
                              <span className="text-[9px] px-1.5 py-0.2 rounded bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-mono">
                                {isEl ? 'Αυτόματη Απάντηση' : 'Auto-Responder'}
                              </span>
                            )}
                          </span>
                            ...
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity mt-2 pt-1 border-t border-black/10 dark:border-white/10 flex items-center justify-end gap-1">
                              <button
                                onClick={() => handleOpenManualSavePrompt(
                                  isMe ? (isEl ? 'Ερώτηση Χρήστη' : 'User Question') : m.text,
                                  isMe ? m.text : ''
                                )}
                                ...
                                title={isEl ? 'Αποθήκευση αυτής της απάντησης για αυτόματη απάντηση AI' : 'Save this response for automatic AI answering'}
                              >
                                <BookmarkPlus className="w-3 h-3" />
                                <span>{isEl ? 'Αποθήκευση ως Auto-Reply' : 'Save as Auto-Reply'}</span>
                              </button>
                            </div>
                          )}
                        </div>
              ...
              >
                <div className="flex items-center justify-between text-xs font-bold text-neutral-800 dark:text-neutral-200 border-b border-neutral-200/20 dark:border-white/5 pb-1.5">
                  <span className="flex items-center gap-1.5">
                    ...
                    {isEl ? 'Εισαγωγή Αποθηκευμένης Απάντησης' : 'Insert Saved Auto-Reply'}
                  </span>
                  <button
                ...
                {(aiSupportQA || []).length === 0 ? (
                  <p className="text-xs text-neutral-400 p-2 italic text-center">
                    {isEl ? 'Δεν υπάρχουν αποθηκευμένες απαντήσεις. Δημιουργήστε μερικές στη Βιβλιοθήκη!' : 'No saved answers yet. Create some in the Auto-Reply Library!'}
                  </p>
                ) : (
            ...
            >
              ...
              {isAdmin && activeChannel === 'support' && (
                <button
                  ...
                  title={isEl ? 'Εισαγωγή αποθηκευμένης αυτόματης απάντησης' : 'Insert saved auto-reply answer'}
                >
              ...
              <input
                ...
                placeholder={
                  activeChannel === 'company' 
                    ? (isEl ? 'Μήνυμα στα μέλη της ομάδας...' : 'Message team members in company channel...')
                    : isAdmin
                    ? (isEl ? 'Απάντηση στο αίτημα χρήστη (θα ερωτηθείτε για αποθήκευση απάντησης AI)...' : 'Reply to user ticket (you will be asked to save answer for AI auto-reply)...')
                    : (isEl ? 'Μήνυμα στην Τεχνική Υποστήριξη ή στο AI...' : 'Message Technical Support staff or AI...')
                }
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-neutral-900 dark:text-white flex items-center gap-1.5">
                      {isEl ? 'Αποθήκευση Απάντησης για Αυτόματη Απάντηση AI;' : 'Save Answer for AI Auto-Reply?'}
                      <Sparkles className="w-4 h-4 text-amber-500" />
                    </h3>
                    <p className="text-[11px] text-neutral-500 dark:text-neutral-400">
                      {isEl ? 'Μάθετε στο AI να απαντά αυτόματα με αυτήν την απάντηση την επόμενη φορά.' : 'Teach the AI to automatically reply with this answer next time.'}
                    </p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleConfirmSaveAutoReply} className="space-y-3.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={isEl ? 'π.χ. Πώς μπορώ να επαναφέρω τον κωδικό μου;' : 'e.g. How do I reset my password?'}
                    className="w-full text-xs glass-input rounded-xl px-3.5 py-2.5 text-neutral-900 dark:text-white"
                    required
                  />
                  <p className="text-[10px] text-neutral-400 mt-0.5">
                    {isEl ? 'Όταν οποιοσδήποτε χρήστης κάνει αυτήν ή παρόμοια ερώτηση, το AI θα απαντά αυτόματα.' : 'When any user asks this or a similar question, the AI will auto-respond.'}
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-700 dark:text-neutral-300 mb-1">
                    {isEl ? 'Κείμενο Αυτόματης Απάντησης' : 'Auto-Reply Answer (What you just sent)'}
                  </label>
                  <textarea
                    rows={4}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-neutral-700 dark:text-neutral-300 mb-1">
                      {isEl ? 'Λέξεις-Κλειδιά (Προαιρετικό)' : 'Trigger Keywords (Optional)'}
                    </label>
                    <input
                      type="text"
                      value={pendingAutoReply.keywords}
                      onChange={(e) => setPendingAutoReply({ ...pendingAutoReply, keywords: e.target.value })}
                      placeholder={isEl ? 'κωδικός, επαναφορά, σύνδεση' : 'password, reset, forgot login'}
                ...
                <div className="pt-3 border-t border-neutral-200/30 dark:border-white/10 flex items-center justify-end gap-2.5">
                  ...
                  >
                    {isEl ? 'Όχι, απλώς αποστολή' : 'No, Just Send'}
                  </button>
                  <button
                    type="submit"
                  ...
                  >
                    ...
                    <span>{isEl ? 'Αποθήκευση στις Απαντήσεις AI' : 'Save to AI Auto-Replies'}</span>
                  </button>
                </div>
              </form>
            </motion.div>
