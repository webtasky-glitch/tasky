...
export const CalendarView: React.FC = () => {
  ...
  const greekDayNames = ['Κυρ', 'Δευ', 'Τρι', 'Τετ', 'Πεμ', 'Παρ', 'Σαβ'];

  const monthNames = language === 'el' ? greekMonthNames : [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'
  ];

  const dayNames = language === 'el' ? greekDayNames : ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
            ...
            <div className="flex items-center gap-1.5 bg-neutral-100 dark:bg-black/40 p-1.5 rounded-2xl border border-neutral-200/80 dark:border-white/5 self-start lg:self-auto overflow-x-auto max-w-full">
              {(['Month', 'Week', 'Day', 'Timeline'] as ViewMode[]).map((mode) => (
                ...
                >
                  ...
                  <span>
                    {language === 'el'
                      ? mode === 'Month' ? 'Μήνας' : mode === 'Week' ? 'Εβδομάδα' : mode === 'Day' ? 'Ημέρα' : 'Χρονοδιάγραμμα'
                      : mode}
                  </span>
                </button>
              ))}
