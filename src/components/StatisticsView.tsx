...
export const StatisticsView: React.FC = () => {
  const { tasks: allTasks, categories, habits: allHabits, currentUserProfile, user, teamMembers } = useTasky() as any;
  const { t, language, tPriority } = useTranslation();
  const isEl = language === 'el';

  // Filter to user and plan specific data
  const tasks = allTasks.filter((t: any) => isTaskForUser(t, currentUserProfile, user, teamMembers));
  ...
  const getPast7DaysLabel = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      dates.push({
        str: `${year}-${month}-${day}`,
        label: d.toLocaleDateString(isEl ? 'el-GR' : 'en-US', { weekday: 'short', day: 'numeric' })
      });
    }
    return dates;
  };
    ...
    <div className="flex-1 glass-panel rounded-[32px] p-6 sm:p-8 space-y-6 overflow-y-auto shadow-xl">
      ...
      <div>
        <h1 className="text-2xl font-sans font-bold text-neutral-900 dark:text-white tracking-tight">
          {isEl ? 'Στατιστικά Παραγωγικότητας' : 'Productivity Statistics'}
        </h1>
        <p className="text-sm text-neutral-500">
          {isEl ? 'Λεπτομερείς αναφορές, ποσοστά ολοκλήρωσης, μετρικές θεμάτων και ιστορικές τάσεις.' : 'In-depth reports, completion percentages, subject metrics, and task historic trends.'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center text-center backdrop-blur-md">
          <h3 className="font-semibold text-neutral-850 dark:text-neutral-100 text-sm mb-4 self-start flex items-center gap-1.5">
            ...
            {isEl ? 'Συνολικό Ποσοστό Ολοκλήρωσης' : 'Overall Completion Rate'}
          </h3>
          ...
          <div className="relative w-36 h-36 flex items-center justify-center">
            ...
            <div className="absolute text-center">
              ...
              <p className="text-[10px] text-neutral-500 dark:text-neutral-400 font-medium tracking-wider uppercase mt-0.5">{isEl ? 'Ολοκληρωμένες' : 'Tasks Done'}</p>
            </div>
          </div>
          ...
          <div className="flex gap-6 mt-5 text-xs text-neutral-500 border-t border-neutral-200/20 dark:border-white/5 pt-4 w-full justify-around font-mono">
            <div>
              <p className="text-neutral-500 dark:text-neutral-400 font-semibold uppercase text-[9px] tracking-wider mb-0.5">{isEl ? 'Ολοκληρωμένες' : 'Completed'}</p>
              <p className="text-sm font-bold text-emerald-500">{completedTasksCount} {isEl ? 'εργασίες' : 'assignments'}</p>
            </div>
            <div className="border-r border-neutral-200/20 dark:border-white/5" />
            <div>
              <p className="text-neutral-500 dark:text-neutral-400 font-semibold uppercase text-[9px] tracking-wider mb-0.5">{isEl ? 'Εκκρεμείς' : 'Pending'}</p>
              <p className="text-sm font-bold text-neutral-700 dark:text-neutral-300">{pendingTasksCount} {isEl ? 'εργασίες' : 'assignments'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 rounded-2xl p-6 shadow-sm backdrop-blur-md">
          <h3 className="font-semibold text-neutral-850 dark:text-neutral-100 text-sm mb-4 flex items-center gap-1.5">
            ...
            {isEl ? 'Εργασίες ανά Προτεραιότητα' : 'Assignments by Priority'}
          </h3>
          ...
          <div className="space-y-4 pt-1">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-rose-500 flex items-center gap-1">● {tPriority('Urgent')}</span>
                <span className="text-neutral-500 dark:text-neutral-400 font-mono">{urgentCount} {isEl ? 'εργασίες' : 'tasks'}</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-orange-500 flex items-center gap-1">● {tPriority('High')}</span>
                <span className="text-neutral-500 dark:text-neutral-400 font-mono">{highCount} {isEl ? 'εργασίες' : 'tasks'}</span>
              </div>
              <div className="w-full bg-white/20 dark:bg-white/10 h-2 rounded-full overflow-hidden">
                <motion.div 
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-amber-500 flex items-center gap-1">● {tPriority('Medium')}</span>
                <span className="text-neutral-500 dark:text-neutral-400 font-mono">{mediumCount} {isEl ? 'εργασίες' : 'tasks'}</span>
              </div>
              <div className="w-full bg-white/20 dark:bg-white/10 h-2 rounded-full overflow-hidden">
                <motion.div 
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-neutral-500 flex items-center gap-1">● {tPriority('Low')}</span>
                <span className="text-neutral-500 dark:text-neutral-400 font-mono">{lowCount} {isEl ? 'εργασίες' : 'tasks'}</span>
              </div>
              <div className="w-full bg-white/20 dark:bg-white/10 h-2 rounded-full overflow-hidden">
                <motion.div 
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 rounded-2xl p-6 shadow-sm flex flex-col justify-between backdrop-blur-md">
          <div>
            <h3 className="font-semibold text-neutral-850 dark:text-neutral-100 text-sm mb-4 flex items-center gap-1.5">
              ...
              {isEl ? 'Τάση Ολοκλήρωσης 7 Ημερών' : '7-Day Completion Trend'}
            </h3>
          ...
          <p className="text-[10px] text-neutral-400 italic text-center mt-4">
            {isEl ? 'Εμφανίζει τον όγκο ολοκληρωμένων εργασιών την προηγούμενη εβδομάδα.' : 'Highlights assignment completion volumes over the preceding week.'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 rounded-2xl p-6 shadow-sm backdrop-blur-md">
          <h3 className="font-semibold text-neutral-850 dark:text-neutral-100 text-sm mb-4 flex items-center gap-1.5">
            ...
            {isEl ? 'Ανάλυση ανά Μάθημα & Κατηγορία' : 'Subject & Category Breakdown'}
          </h3>
              ...
              <div key={idx} className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono text-neutral-500 dark:text-neutral-400 font-semibold">
                    {stat.completed} {isEl ? 'ολοκληρωμένες' : 'completed'} / {stat.total} {isEl ? 'σύνολο' : 'total'}
                  </span>
                </div>
                ...
                <div className="relative w-full bg-white/20 dark:bg-white/10 h-4 rounded-full overflow-hidden flex items-center border border-white/25 dark:border-white/5">
                  ...
                  />
                  {stat.total === 0 && (
                    <span className="absolute left-3 text-[9px] text-neutral-500 dark:text-neutral-400 italic">{isEl ? 'Καμία ενεργή εργασία' : 'No assignments active'}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="glass-card border border-white/25 dark:border-white/5 rounded-2xl p-6 shadow-sm flex flex-col justify-between backdrop-blur-md">
          <div>
            <h3 className="font-semibold text-neutral-850 dark:text-neutral-100 text-sm mb-4 flex items-center gap-1.5">
              ...
              {isEl ? 'Δείκτης Συνέπειας Στόχων & Συνηθειών' : 'Goals & Habits Consistency Index'}
            </h3>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 py-4">
              ...
              <div className="space-y-2 text-xs">
                <h4 className="font-bold text-neutral-850 dark:text-neutral-200">{isEl ? 'Πόσο συνεπείς είστε;' : 'How consistent are you?'}</h4>
                <p className="text-neutral-500 dark:text-neutral-400 leading-relaxed">
                  {isEl ? (
                    <>Η βαθμολογία συνέπειας στόχων και συνηθειών σας είναι <b>{habitCompletionRate}%</b>, προερχόμενη από τους δείκτες σερί των ενεργών συνηθειών σας. Η καθημερινή ολοκλήρωση ενισχύει αυτόν τον δείκτη παραγωγικότητας.</>
                  ) : (
                ...
                <div className="flex gap-2 pt-1 font-mono text-[10px] text-neutral-500 dark:text-neutral-400">
                  <span className="bg-white/20 dark:bg-white/10 px-2 py-0.5 rounded">
                    🔥 {isEl ? 'Συνολικά Σερί:' : 'Total Streaks:'} {habits.reduce((acc, h) => acc + h.streak, 0)} {isEl ? 'ημέρες' : 'days'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/10 dark:bg-black/20 p-3 rounded-xl border border-white/20 dark:border-white/5 flex items-center gap-2.5 text-[10px] text-neutral-600 dark:text-neutral-400 leading-normal">
            ...
            <span>{isEl ? 'Τα στατιστικά υπολογίζονται τοπικά και συγχρονίζονται άμεσα μεταξύ των χώρων εργασίας σας.' : 'Statistics are computed locally and sync instantly across spaces when your backup synchronization is active.'}</span>
          </div>
        </div>
      </div>
    </div>
