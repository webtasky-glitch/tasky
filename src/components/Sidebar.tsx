...
export const Sidebar: React.FC<{ onClose?: () => void; isMobile?: boolean }> = ({ onClose, isMobile }) => {
  ...
  return (
    <aside className={`flex flex-col h-full overflow-y-auto shrink-0 select-none ${isMobile ? 'w-full bg-[#f4f5fa] dark:bg-[#12131f]' : 'w-64 glass-panel rounded-[32px] shadow-xl'}`}>
      {/* Brand Header */}
      ...
      {userOrganizations && userOrganizations.length > 0 && (
        <div className="px-4 py-2.5 border-b border-neutral-200/20 dark:border-white/5 bg-indigo-500/5">
          <button
            onClick={() => setIsWorkspaceSelectorOpen(true)}
            className="w-full p-2 bg-white/60 hover:bg-white dark:bg-neutral-800/60 dark:hover:bg-neutral-800 border border-neutral-200/60 dark:border-white/10 rounded-2xl flex items-center justify-between text-left transition-all cursor-pointer group shadow-sm"
          >
