...
export const SourceSelector: React.FC<SourceSelectorProps> = ({ onAddSource }) => {
  ...
  const processFile = (file: File) => {
    const maxLimit = 1024 * 1024 * 1024; // 1 GB Limit
    if (file.size > maxLimit) {
      alert(isEl ? 'Το αρχείο είναι πολύ μεγάλο! Το μέγιστο επιτρεπόμενο μέγεθος είναι 1 GB.' : 'File is too large! Maximum allowed upload size is 1 GB.');
      return;
    }
    ...
    if (file.size <= 800 * 1024) {
      ...
      onAddSource(file.name, extension, sizeStr, objectUrl);
    }
  };
  ...
  const handleAddEmail = (e?: React.FormEvent | React.KeyboardEvent) => {
    ...
    const senderInfo = emailSender.trim() 
      ? (isEl ? `Από: ${emailSender.trim()}` : `From: ${emailSender.trim()}`) 
      : (isEl ? 'Αναφορά Email' : 'Email Reference');
  ...
  const handleAddLink = (e?: React.FormEvent | React.KeyboardEvent) => {
    ...
    onAddSource(`🔗 ${linkTitle.trim()}`, 'link', isEl ? 'Σύνδεσμος Web' : 'Web URL', url);
    setLinkTitle('');
    setLinkUrl('');
  };
    ...
    <div className="bg-neutral-50/50 dark:bg-neutral-900/40 border border-neutral-200/50 dark:border-white/5 rounded-2xl p-4 space-y-4">
      <div className="flex items-center justify-between border-b border-neutral-200/30 dark:border-white/5 pb-2">
        <span className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">
          {isEl ? 'Προσθήκη Πηγής Εργασίας' : 'Add Task Source'}
        </span>
        ...
        <div className="space-y-3.5">
          ...
          >
            ...
            <p className="text-[11px] text-neutral-500 dark:text-neutral-400 text-center">
              {isEl ? (
                <>Σύρετε το αρχείο εδώ, ή <label className="text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer">περιηγηθείτε<input type="file" className="hidden" onChange={handleFileSelect} /></label></>
              ) : (
          ...
          <div className="relative flex items-center my-1.5 text-center">
            ...
            <span className="flex-shrink mx-3 text-[9px] text-neutral-400 font-medium uppercase tracking-wider">
              {isEl ? 'ή εισαγάγετε χειροκίνητα' : 'or specify manually'}
            </span>
            <div className="flex-grow border-t border-neutral-200 dark:border-neutral-800"></div>
          </div>
          ...
          <div className="flex gap-2 items-end">
            <div className="flex-1 space-y-1">
              <label className="text-[10px] text-neutral-400 font-medium">
                {isEl ? 'Όνομα Αρχείου' : 'File Name'}
              </label>
              <input
                type="text"
                placeholder={isEl ? 'π.χ. programma_spoudon.pdf' : 'e.g. syllabus_physics.pdf'}
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="text-[10px] text-neutral-400 font-medium">
                {isEl ? 'Θέμα Email' : 'Email Subject'}
              </label>
              <input
                type="text"
                required
                placeholder={isEl ? 'π.χ. Σχόλια για το Εργαστήριο 3' : 'e.g. Feedback on Lab 3'}
                value={emailSubject}
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-neutral-400 font-medium">
                {isEl ? 'Αποστολέας / Από' : 'Sender / From'}
              </label>
              <input
                type="text"
                placeholder={isEl ? 'π.χ. kathigitis@panepistimio.gr' : 'e.g. prof.smith@university.edu'}
                value={emailSender}
          ...
          <div className="space-y-1">
            <label className="text-[10px] text-neutral-400 font-medium">
              {isEl ? 'Σύντομο Απόσπασμα / Σημείωση' : 'Brief Snippet / Highlight'}
            </label>
            <input
              type="text"
              placeholder={isEl ? "π.χ. 'Παρακαλώ ελέγξτε την ενότητα 3...'" : "e.g. 'Please check section 3 and re-submit by Friday...'"}
              value={emailPreview}
          ...
          <div className="flex justify-end pt-1">
            ...
            >
              ...
              {isEl ? 'Προσθήκη Πηγής Email' : 'Add Email Source'}
            </button>
          </div>
        </div>
      )}
      ...
      {activeTab === 'link' && (
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="text-[10px] text-neutral-400 font-medium">
                {isEl ? 'Τίτλος / Όνομα Συνδέσμου' : 'Link Title / Name'}
              </label>
              <input
                type="text"
                required
                placeholder={isEl ? 'π.χ. Κοινόχρηστο Έγγραφο Google' : 'e.g. Shared Google Doc'}
                value={linkTitle}
          ...
          <div className="flex justify-end pt-1">
            ...
            >
              <Plus className="w-3.5 h-3.5" />
              {isEl ? 'Προσθήκη Συνδέσμου' : 'Add Link Source'}
            </button>
          </div>
        </div>
      )}
