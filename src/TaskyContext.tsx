...
export const TaskyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 2. Database initialization and cleanup helper (Enforces keeping ONLY Admin and 0 plans)
  useEffect(() => {
    const initializeAndCleanupDatabase = async () => {
      try {
        setSyncStatus('syncing');

        // 1. Delete ALL organizations / plans
        const orgSnap = await getDocs(collection(db, 'organizations'));
        for (const orgDoc of orgSnap.docs) {
          try {
            await deleteDoc(doc(db, 'organizations', orgDoc.id));
          } catch (e) {
            console.error("Error deleting org during cleanup:", e);
          }
        }

        // 2. Delete ALL non-admin users from team collection
        const teamSnap = await getDocs(collection(db, 'team'));
        let adminExistsInTeam = false;

        for (const teamDoc of teamSnap.docs) {
          const tmData = teamDoc.data() as TeamMember;
          const isSpiderEmail = tmData.email && tmData.email.toLowerCase().trim() === 'spidereg2010@gmail.com';
          const isAdminRank = tmData.rank === 'Admin' || isSpiderEmail;

          if (!isAdminRank) {
            try {
              await deleteDoc(doc(db, 'team', teamDoc.id));
            } catch (e) {
              console.error("Error deleting non-admin team member:", e);
            }
          } else {
        ...
        try {
          ...
          console.error("Error clearing join requests:", e);
        }

        // 4. Seed categories, habits, and tasks if empty
  ...
  const activeUserId = currentUserProfile?.id || (user ? (user.email === 'spidereg2010@gmail.com' ? 'admin-spidereg2010' : `tm-fb-${user.uid}`) : 'admin-spidereg2010');
  ...
  const userCategories = useMemo(() => {
    ...
    return allCategories.filter(c => {
      ...
      return activeUserId === 'admin-spidereg2010';
    });
  ...
  useEffect(() => {
    ...
    const userHasCat = allCategories.some(c => c.userId === activeUserId || (!c.userId && activeUserId === 'admin-spidereg2010'));
