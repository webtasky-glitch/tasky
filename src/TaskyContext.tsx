...
interface TaskyContextType {
  ...
  activeTab: 'dashboard' | 'calendar' | 'habits' | 'statistics' | 'team' | 'organizations' | 'chat' | 'create-member' | 'projects' | 'admin-requests' | 'privacy' | string;
  ...
  setOrganizationManager: (orgId: string, managerMemberId: string) => Promise<void>;
  updateMemberRankInOrg: (memberId: string, orgId: string, newRank: UserRank, newRole?: string) => Promise<void>;
  updateMemberPlanRole: (memberId: string, orgId: string, newRole: string) => Promise<void>;
  updatePlanRolePermissions: (orgId: string, roleName: string, permissions: PlanPermissions) => Promise<void>;
  sendAvailabilityInquiry: (recipientMemberId: string, messageText: string) => Promise<void>;
  ...
  updatePrivacyConsents: (consents: Partial<PrivacyConsents>) => Promise<void>;
  auditLogs: AuditLog[];
  logAuditEvent: (log: Omit<AuditLog, 'id' | 'timestamp'>) => Promise<void>;
  dataRetentionPolicy: DataRetentionPolicy;
  updateDataRetentionPolicy: (policy: Partial<DataRetentionPolicy>) => Promise<void>;
  exportUserData: () => Promise<void>;
  deleteUserAccountWorkflow: (options: { anonymizeTeamHistory: boolean }) => Promise<{ success: boolean; certificateId: string }>;
...
export const TaskyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // GDPR & Privacy Center State
  const [privacyConsents, setPrivacyConsents] = useState<PrivacyConsents>(() => {
    try {
      const saved = localStorage.getItem('tasky_privacy_consents');
      return saved ? JSON.parse(saved) : {
  ...
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>(() => {
    try {
      const saved = localStorage.getItem('tasky_audit_logs');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  ...
  const [dataRetentionPolicy, setDataRetentionPolicy] = useState<DataRetentionPolicy>(() => {
    try {
      const saved = localStorage.getItem('tasky_retention_policy');
      return saved ? JSON.parse(saved) : {
        retentionDays: 0,
        autoArchiveCompletedTasks: false,
        autoPurgeAuditLogsDays: 90
      };
    } catch {
      return {
        retentionDays: 0,
  ...
  useEffect(() => {
    ...
    if (chosenMember) {
      ...
      const savedGuestProfile = localStorage.getItem('tasky_guest_profile');
  ...
  // Sync Trigger (manually refreshes status visualizer and flushes pending mutations)
  const triggerSync = () => {
    if (!navigator.onLine) {
      setSyncStatus('offline');
      return;
  ...
  const updateTeamMember = async (member: TeamMember) => {
    if (member.id === 'tm-guest' || localStorage.getItem('tasky_guest_mode') === 'true') {
      localStorage.setItem('tasky_guest_profile', JSON.stringify(member));
  ...
  // Plan Manager or Admin updates a user's role specifically for ONE plan without touching other plans
  const updateMemberPlanRole = async (memberId: string, orgId: string, newRole: string) => {
    const targetMember = teamMembers.find(tm => tm.id === memberId || (tm.email && tm.email.toLowerCase().trim() === memberId.toLowerCase().trim()));
    if (!targetMember) throw new Error('Target team member not found');

    const currentOrgRoles = targetMember.orgRoles || {};
    ...
    const currentOrgRanks = targetMember.orgRanks || {};
    const isManagerRole = newRole.toLowerCase() === 'manager';
    const updatedOrgRanks: Record<string, UserRank> = {
      ...currentOrgRanks,
      [orgId]: isManagerRole ? 'Manager' : 'User'
    };
    ...
    const updatedMember: TeamMember = {
      ...
      orgRanks: updatedOrgRanks,
      // If currently active in this plan, update displayed role
      ...(targetMember.orgId === orgId ? { role: newRole, rank: isManagerRole ? 'Manager' : 'User' } : {})
    };
    ...
    if (currentUserProfile && (currentUserProfile.id === targetMember.id || currentUserProfile.email?.toLowerCase().trim() === targetMember.email?.toLowerCase().trim())) {
      setCurrentUserProfile(prev => prev ? {
        ...
        orgRoles: updatedOrgRoles,
        orgRanks: updatedOrgRanks,
        ...(prev.orgId === orgId ? { role: newRole, rank: isManagerRole ? 'Manager' : 'User' } : {})
      } : null);
    }
    ...
    try {
      ...
      console.error("Error updating member plan role:", err);
      setSyncStatus('error');
      throw err;
    }
  };

  // Plan Manager or Admin configures permissions for a specific role inside a plan
  const updatePlanRolePermissions = async (orgId: string, roleName: string, permissions: PlanPermissions) => {
    const targetOrg = organizations.find(o => o.id === orgId);
    if (!targetOrg) throw new Error('Plan organization not found');

    const currentRolePerms = targetOrg.rolePermissions || {};
    const updatedOrg: Organization = {
      ...targetOrg,
      rolePermissions: {
        ...currentRolePerms,
    ...
    try {
      ...
      console.error("Error updating plan role permissions:", err);
      setSyncStatus('error');
      throw err;
    }
  };

  // Send direct message inquiry about task availability
  const sendAvailabilityInquiry = async (recipientMemberId: string, messageText: string) => {
    const senderId = currentUserProfile?.id || user?.uid || 'user-sender';
    const senderName = currentUserProfile?.name || user?.displayName || 'Team Colleague';
    const senderAvatar = currentUserProfile?.avatar || '👤';
    const senderEmail = currentUserProfile?.email || user?.email || '';
    ...
    try {
      ...
      console.error("Error dispatching availability inquiry message:", err);
      setSyncStatus('error');
      throw err;
    }
  };
  ...
  // GDPR & Privacy Center Handlers
  const updatePrivacyConsents = async (newConsents: Partial<PrivacyConsents>) => {
    const updated: PrivacyConsents = {
      ...privacyConsents,
      ...newConsents,
      essentialContract: true, // Immutable under Art. 6(1)(b)
      updatedAt: new Date().toISOString()
    };
    setPrivacyConsents(updated);
    localStorage.setItem('tasky_privacy_consents', JSON.stringify(updated));
    if (user?.uid && db) {
      setDoc(doc(db, 'privacyRecords', user.uid), {
        consents: updated,
        userId: user.uid,
        userEmail: user.email || '',
        updatedAt: updated.updatedAt
      }).catch(err => console.error('Firestore consent update error:', err));
    }
  };

  const logAuditEvent = async (log: Omit<AuditLog, 'id' | 'timestamp'>) => {
    const newLog: AuditLog = {
      ...log,
      id: 'log-' + Math.random().toString(36).substring(2, 9),
      actorId: log.actorId || currentUserProfile?.id || user?.uid || 'anon',
      actorEmail: log.actorEmail || user?.email || currentUserProfile?.email || 'user@tasky.local',
      timestamp: new Date().toISOString()
    };
    setAuditLogs(prev => {
      const updated = [newLog, ...prev.slice(0, 99)];
      localStorage.setItem('tasky_audit_logs', JSON.stringify(updated));
      return updated;
    });
    if (db) {
      setDoc(doc(db, 'auditLogs', newLog.id), newLog).catch(err => console.error('Firestore audit log error:', err));
    }
  };
  ...
  const updateDataRetentionPolicy = async (policy: Partial<DataRetentionPolicy>) => {
    ...
    localStorage.setItem('tasky_retention_policy', JSON.stringify(updated));
  };

  const updateFamilyRole = async (memberId: string, role: FamilyRole) => {
    setTeamMembers(prev => prev.map(m => {
      if (m.id === memberId) {
        const updated = { ...m, familyRole: role };
        if (db) setDoc(doc(db, 'team', m.id), updated).catch(e => console.error(e));
        return updated;
      }
      return m;
    }));
    if (currentUserProfile && currentUserProfile.id === memberId) {
      setCurrentUserProfile(prev => prev ? ({ ...prev, familyRole: role }) : null);
    }
    await logAuditEvent({
      action: 'modify_role',
      resourceType: 'profile',
      resourceId: memberId,
      details: `Family protection role updated to ${role}`
    });
  };

  const reportSecurityIncident = async (incident: Omit<SecurityIncident, 'id' | 'detectedAt'>) => {
    ...
    setSecurityIncidents(prev => {
      ...
      localStorage.setItem('tasky_security_incidents', JSON.stringify(updated));
      return updated;
    });
    if (db) {
      setDoc(doc(db, 'securityIncidents', newIncident.id), newIncident).catch(e => console.error(e));
    }
  };

  const exportUserData = async () => {
    const userId = currentUserProfile?.id || user?.uid || 'user';
    ...
    const userTasks = tasks.filter(t => {
      ...
      return t.createdBy === userId || (user?.uid && t.createdBy === user.uid) || ids.includes(userId) || (user?.uid && ids.includes(user.uid));
    });

    const userHabits = habits.filter(h => !h.userId || h.userId === userId || (user?.uid && h.userId === user.uid));
    const userProjects = projects.filter(p => p.ownerId === userId || (p.memberIds && p.memberIds.includes(userId)));
    const userMessages = messages.filter(m => m.senderEmail === userEmail || m.senderId === userId);

    const payload = {
      gdprNotice: "Personal Data Portability Archive generated under Regulation (EU) 2016/679 (GDPR), Article 20.",
      controller: "Tasky Platform / Client Workspace",
      exportTimestamp: new Date().toISOString(),
      userProfile: {
        pseudonymisedId: userId,
        email: userEmail,
        name: currentUserProfile?.name || 'User',
        rank: currentUserProfile?.rank || 'User',
        familyRole: currentUserProfile?.familyRole || 'parent',
        joinedAt: currentUserProfile?.joinedAt || new Date().toISOString()
      },
      privacyConsents,
      records: {
      ...
      auditHistory: auditLogs.filter(l => l.actorId === userId || l.actorEmail === userEmail)
    };
    ...
    a.href = url;
    a.download = `tasky-personal-data-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const deleteUserAccountWorkflow = async (options: { anonymizeTeamHistory: boolean }) => {
    const userId = currentUserProfile?.id || user?.uid || 'user';
    const certId = 'GDPR-ERASURE-' + Math.random().toString(36).substring(2, 9).toUpperCase();

    if (options.anonymizeTeamHistory) {
      setTasks(prev => prev.map(t => {
        let modified = false;
        const newComments = t.comments?.map(c => {
          if (c.authorId === userId || (currentUserProfile && c.authorName === currentUserProfile.name)) {
            modified = true;
            return { ...c, authorName: 'Former Team Member', authorId: 'anonymized-user' };
          }
          return c;
        }) || [];

        let newCreatedBy = t.createdBy;
        if (t.createdBy === userId || (user?.uid && t.createdBy === user.uid)) {
          newCreatedBy = 'anonymized-user';
          modified = true;
        }

        if (modified) {
          const updated = { ...t, comments: newComments, createdBy: newCreatedBy };
          if (db) setDoc(doc(db, 'tasks', t.id), updated).catch(e => console.error(e));
          return updated;
        }
        return t;
      }));
      ...
      setTasks(prev => prev.filter(t => {
        const isMyTask = t.createdBy === userId || (user?.uid && t.createdBy === user.uid);
        if (isMyTask && (!t.orgId || t.orgId === 'personal')) {
          if (db) deleteDoc(doc(db, 'tasks', t.id)).catch(e => console.error(e));
          return false;
        }
        return true;
      }));
    ...
    setHabits(prev => prev.filter(h => {
      if (!h.userId || h.userId === userId || (user?.uid && h.userId === user.uid)) {
        if (db) deleteDoc(doc(db, 'habits', h.id)).catch(e => console.error(e));
        return false;
      }
      return true;
    }));
    ...
    if (currentUserProfile) {
      setTeamMembers(prev => prev.filter(m => m.id !== currentUserProfile.id));
      if (db) deleteDoc(doc(db, 'team', currentUserProfile.id)).catch(e => console.error(e));
    }
    ...
    await logAuditEvent({
      ...
      resourceType: 'profile',
      resourceId: userId,
      details: `Erasure completed. Certificate: ${certId}. AnonymizeTeamHistory: ${options.anonymizeTeamHistory}`
    });
