...
export interface Task {
  ...
  startTime?: string; // e.g. "09:00" (optional task time)
  endTime?: string; // e.g. "10:30" (optional end time)
  isAllDay?: boolean; // true when no specific hour is scheduled
...
export type PlanRole = 'Manager' | 'Developer' | 'Designer' | 'Member' | 'Viewer' | string;
...
export const DEFAULT_ROLE_PERMISSIONS: Record<string, PlanPermissions> = {
  Manager: {
    canCreateTasks: true,
...
export interface Organization {
  ...
  managerName?: string;
  managerEmail?: string;
  rolePermissions?: Record<string, PlanPermissions>; // Custom plan role permissions
}
...
export interface TeamMember {
  ...
  orgRoles?: Record<string, PlanRole>; // Plan-specific roles: Manager, Developer, Designer, Member, Viewer, etc.
