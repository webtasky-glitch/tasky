...
export const isTaskForUser = (
  ...
  if (currentUserProfile.orgId) {
    ...
    // GDPR Family Child Protection: Children ONLY see tasks assigned to them specifically
    if (currentUserProfile.familyRole === 'child') {
      return isTaskAssignedToUser(task, currentUserProfile.id, user?.uid);
    }
