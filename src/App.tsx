...
const AppContent: React.FC = () => {
  ...
  const renderActiveView = () => {
    switch (activeTab) {
      ...
      case 'all-tasks':
        return <AllTasksView key="all-tasks" />;
      case 'timeline':
        return <TimelineScheduleView key="timeline" />;
      case 'projects':
      ...
      case 'privacy':
        return <PrivacyCenterView key="privacy" />;
      default:
        return <DashboardView key="dashboard" />;
    }
  };
  ...
  const mobileTabs = [
    { id: 'dashboard', name: t('sidebar.dashboard') || (language === 'el' ? 'Εργασίες' : 'Tasks'), icon: CheckSquare },
    { id: 'projects', name: t('sidebar.projects') || (language === 'el' ? 'Έργα' : 'Projects'), icon: FolderKanban },
    { id: 'calendar', name: t('sidebar.calendar') || (language === 'el' ? 'Ημερολόγιο' : 'Calendar'), icon: Calendar },
    { id: 'habits', name: t('sidebar.habits') || (language === 'el' ? 'Συνήθειες' : 'Habits'), icon: Flame },
    { id: 'chat', name: t('sidebar.chat') || (language === 'el' ? 'Συνομιλία' : 'Chat'), icon: MessageSquare },
  ];
    ...
    <div className="flex h-screen w-screen bg-gradient-to-br from-[#e0e4f5] via-[#f5e1e7] to-[#e8eaf6] dark:from-[#1a1c2c] dark:via-[#4a192c] dark:to-[#12141d] text-neutral-800 dark:text-neutral-100 font-sans overflow-hidden p-3 gap-3 md:p-6 md:gap-6 flex-col md:flex-row transition-all duration-500">
      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 bg-white/40 dark:bg-neutral-900/40 backdrop-blur-md rounded-2xl border border-white/20 dark:border-white/5 shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Clickable Mobile User Profile Avatar */}
          <button
            type="button"
            onClick={() => setIsProfileModalOpen(true)}
            className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-900 flex items-center justify-center text-xs font-bold text-indigo-600 dark:text-indigo-400 overflow-hidden shadow-sm hover:scale-105 active:scale-95 transition-all cursor-pointer shrink-0"
            title={language === 'el' ? 'Επεξεργασία Προφίλ' : 'Edit Profile'}
          >
            {currentUserProfile?.avatar && currentUserProfile.avatar.startsWith('data:image') ? (
              <img 
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {(!isOnline || syncStatus === 'offline') && (
          <div className="bg-amber-500/15 dark:bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3 mb-3 flex items-center justify-between gap-3 shrink-0 text-amber-900 dark:text-amber-200 shadow-sm">
            <div className="flex items-center gap-2.5 min-w-0">
              <CloudOff className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 animate-pulse" />
              <p className="text-xs font-medium leading-tight truncate">
                <span className="font-bold">{language === 'el' ? 'Λειτουργία Εκτός Σύνδεσης:' : 'Offline Mode Active:'}</span> {language === 'el' ? 'Οι αλλαγές αποθηκεύονται τοπικά στον περιηγητή και θα συγχρονιστούν αυτόματα όταν επανέλθει η σύνδεση.' : 'Your edits and task updates are saved locally in browser storage and will automatically sync once your connection is restored.'}
              </p>
            </div>
            <button
              onClick={triggerSync}
              className="px-2.5 py-1 bg-amber-500 hover:bg-amber-600 text-white font-bold text-[10px] uppercase rounded-xl transition-all cursor-pointer shadow-sm shrink-0 flex items-center gap-1"
            >
              <CloudLightning className="w-3 h-3" />
              <span>{language === 'el' ? 'Επανάληψη Συγχρονισμού' : 'Retry Sync'}</span>
            </button>
          </div>
        )}
          ...
          <div className="bg-amber-500/10 dark:bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 mb-4 flex items-center justify-between gap-4 shrink-0 text-amber-800 dark:text-amber-300">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold leading-normal">
                {language === 'el' ? `Προσομοίωση ως ${currentUserProfile.name} (${currentUserProfile.rank}). Όλες οι προβολές ενεργούν για αυτό το προφίλ.` : `Impersonating ${currentUserProfile.name} (${currentUserProfile.rank} role). All views and actions act on this user profile.`}
              </p>
            </div>
            <button
            ...
            >
              {language === 'el' ? 'Επιστροφή σε Διαχειριστή' : 'Switch back to Admin'}
            </button>
          </div>
        )}
